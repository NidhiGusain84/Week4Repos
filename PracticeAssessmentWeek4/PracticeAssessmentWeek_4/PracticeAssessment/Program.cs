...
namespace PracticeAssessment
{
    class Pants
    {
        ...
        {
            return $"\nStyle: {_style}, Sizes: {string.Join(" ", _sizes)}";
        }
    ...
    class Program
    {
        ...
        {
            List<Pants> pantsList = CreatePantsList();
            ...
            {               
                ...
                switch (input)
                {
                    case 1:
                        ...
                        break;
                    case 2:
                        CheckAvailability(pantsList);
                        break;
                    case 3:
                        DeletePant(pantsList);
                        break;
                    case 4:
                        ListAllPants(pantsList);
                        break;
        ...
        static List<Pants> CreatePantsList()
        {
        ...
        static void ListAllPants(List<Pants> pantList)
        {
        ...
        static void CheckAvailability(List<Pants> pants)
        {
            Console.WriteLine("Please enter Style");
            string style = Console.ReadLine();
            ...
            {
                if (item.GetStyle().ToLower() == style.ToLower() && item.IsAvailable(size))
                {
                    itemFound = true;
                    break;
                }
        ...
        private static Pants FindPantByStyle(List<Pants> pantsList, string style)
        {
            foreach (Pants item in pantsList)
            {
                if (item.GetStyle().ToLower() == style.ToLower())
                {
                    return item;
                }
            }
